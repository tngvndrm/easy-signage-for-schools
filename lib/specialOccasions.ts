...
export async function readSpecialOccasions(
  ...
  for (const row of rows.slice(header.firstDataRow)) {
    ...
    const displayFromMin: number | null =
      parseClock(rawFromTime) ?? lastDisplayFromMin;
    const displayToMin: number | null =
      parseClock(rawToTime) ?? lastDisplayToMin;
    ...
    // `!== null`, not truthiness: midnight is 0 and still a real boundary.
    if (displayFromMin !== null) lastDisplayFromMin = displayFromMin;
    if (displayToMin !== null) lastDisplayToMin = displayToMin;
    if (rawBigSlide) {
